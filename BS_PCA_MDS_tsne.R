...
input_file_name = "all_methylKit_fix_AMFT_props.csv" #### specify name of input file here
dat1 = read.csv(input_file_name, check.names=FALSE, stringsAsFactors=FALSE) # takes a few mins to read the full file
...
P_PCA_v1   <- plot_PCA_1(PCA_res2,dat_sample_info,  paste("N CpG =", length(dat2[,1])))
P_PCA_v2   <- plot_PCA_2(PCA_res2,dat_sample_info,  paste("N CpG =", length(dat2[,1])))
...
P_tsne_alls_v1   <- plot_tsne_1(tsne_model_1, dat_sample_info, paste("N CpG =", length(dat2[,1])))
P_tsne_alls_v2   <- plot_tsne_2(tsne_model_1, dat_sample_info, paste("N CpG =", length(dat2[,1])))


######## output

dir.create("Locus_level_plots")
setwd("Locus_level_plots")

pdf("PCA_Locus_level.pdf", width = 21, height = 10.5)
plot_grid(P_PCA_v1, P_PCA_v2, labels=c("A", "B"), ncol = 2, nrow = 1)
dev.off()
getwd() ## where has my plot gone....?

pdf("tSNE_Locus_level.pdf", width = 21, height = 10.5)
...
writeLines(capture.output(sessionInfo()), "BS_PCA_MDS_tsne.R_sessionInfo.txt")
